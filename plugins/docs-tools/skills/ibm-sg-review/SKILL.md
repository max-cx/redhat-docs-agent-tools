...
context: fork
name: ibm-sg-review
description: Parallel PDF style review workflow using IBM Style Guide chunks. Reviews PRs, commits, or files against PDF style guide rules. Use with arguments like PR URLs (#123), commit refs (HEAD~1), or file paths. Spawns parallel agents to analyze content against style guide chunks for faster review.
---

<!--
You can use this skill with the following arguments:

- /ibm-sg-review https://github.com/org/repo/pull/123 - review a PR by URL
- /ibm-sg-review #123 or /ibm-sg-review 123 - review a PR by number
- /ibm-sg-review HEAD~1 - review a commit
- /ibm-sg-review path/to/file.adoc - review a file
- /ibm-sg-review - review the latest commit (default)
-->
...
2. Lists all chunk files in the plugins/docs-tools/skills/ibm-sg-review/chunks/ directory
...
1. Run the following command to check for chunk files:
   ```bash
   ls ~/redhat-docs-agent-tools/plugins/docs-tools/skills/ibm-sg-review/chunks/ 2>/dev/null | head -1
   ```
...
3. **If chunks are missing** (command returns nothing or errors):
   ...
   c. Create the style-guides directory and get the full absolute path:
      ```bash
      mkdir -p ~/redhat-docs-agent-tools/plugins/docs-tools/skills/ibm-sg-review/style-guides && realpath ~/redhat-docs-agent-tools/plugins/docs-tools/skills/ibm-sg-review/style-guides/
      ```
   ...
   e. If user uploaded PDFs, process them:
      ```bash
      mkdir -p ~/redhat-docs-agent-tools/plugins/docs-tools/skills/ibm-sg-review/chunks
      ```

      For each PDF file found in the style-guides directory:
      ```bash
      cd ~/redhat-docs-agent-tools/plugins/docs-tools/skills/ibm-sg-review/style-guides && for pdf in *.pdf; do
        if [[ -f "$pdf" ]]; then
          basename="${pdf%.pdf}"
          echo "Processing $pdf..."
   ...
   f. Verify chunks were created:
      ```bash
      ls ~/redhat-docs-agent-tools/plugins/docs-tools/skills/ibm-sg-review/chunks/ | wc -l
      ```
...
2. List all chunk files in `plugins/docs-tools/skills/ibm-sg-review/chunks/` directory and its subdirectories:
   ```
   ls ~/redhat-docs-agent-tools/plugins/docs-tools/skills/ibm-sg-review/chunks/
   ```
   Store the list of chunk filenames for spawning agents.

3. Create the temporary directory if it doesn't exist:
   ```bash
   mkdir -p ~/redhat-docs-agent-tools/plugins/docs-tools/skills/ibm-sg-review/temp
   ```

4. Retrieve the commit diff content and save it to a temporary file:
   ```
   ~/redhat-docs-agent-tools/plugins/docs-tools/skills/ibm-sg-review/temp/commit-diff.txt
   ```
...
1. Read the chunk file: ~/redhat-docs-agent-tools/plugins/docs-tools/skills/ibm-sg-review/chunks/<CHUNK_FILENAME>
2. Read the commit diff: ~/redhat-docs-agent-tools/plugins/docs-tools/skills/ibm-sg-review/temp/commit-diff.txt
...
4. Clean up temporary files:
   ```bash
   rm -rf ~/redhat-docs-agent-tools/plugins/docs-tools/skills/ibm-sg-review/temp
   ```
